...
import com.github.paladijn.d2rsavegameparser.internal.parser.ItemScaffolding;
import com.github.paladijn.d2rsavegameparser.model.CharacterType;
import com.github.paladijn.d2rsavegameparser.model.Item;
import com.github.paladijn.d2rsavegameparser.model.ItemContainer;
import com.github.paladijn.d2rsavegameparser.model.ItemLocation;
import com.github.paladijn.d2rsavegameparser.model.ItemPosition;
...
import com.github.paladijn.d2rsavegameparser.model.ItemType;
import com.github.paladijn.d2rsavegameparser.txt.ArmorStats;
import com.github.paladijn.d2rsavegameparser.txt.ItemStatCost;
import com.github.paladijn.d2rsavegameparser.txt.MagicAffix;
import com.github.paladijn.d2rsavegameparser.txt.MiscStats;
import com.github.paladijn.d2rsavegameparser.txt.SetItem;
import com.github.paladijn.d2rsavegameparser.txt.TXTProperties;
import com.github.paladijn.d2rsavegameparser.txt.UniqueItem;
import com.github.paladijn.d2rsavegameparser.txt.WeaponStats;
...
import static com.github.paladijn.d2rsavegameparser.internal.parser.ParseHelper.PROPERTY_CHARGED_SKILL;
import static com.github.paladijn.d2rsavegameparser.internal.parser.ParseHelper.PROPERTY_COLD_MIN_DMG;
import static com.github.paladijn.d2rsavegameparser.internal.parser.ParseHelper.PROPERTY_END;
import static com.github.paladijn.d2rsavegameparser.internal.parser.ParseHelper.PROPERTY_FIRE_MIN_DMG;
import static com.github.paladijn.d2rsavegameparser.internal.parser.ParseHelper.PROPERTY_LIGHT_MIN_DMG;
import static com.github.paladijn.d2rsavegameparser.internal.parser.ParseHelper.PROPERTY_MAGIC_MIN_DMG;
import static com.github.paladijn.d2rsavegameparser.internal.parser.ParseHelper.PROPERTY_PHYS_MAX_DMG;
import static com.github.paladijn.d2rsavegameparser.internal.parser.ParseHelper.PROPERTY_POISON_MIN_DMG;
import static com.github.paladijn.d2rsavegameparser.internal.parser.ParseHelper.PROPERTY_SKILL_ATTACK;
import static com.github.paladijn.d2rsavegameparser.internal.parser.ParseHelper.PROPERTY_SKILL_DEATH;
import static com.github.paladijn.d2rsavegameparser.internal.parser.ParseHelper.PROPERTY_SKILL_GET_HIT;
import static com.github.paladijn.d2rsavegameparser.internal.parser.ParseHelper.PROPERTY_SKILL_HIT;
import static com.github.paladijn.d2rsavegameparser.internal.parser.ParseHelper.PROPERTY_SKILL_KILL;
import static com.github.paladijn.d2rsavegameparser.internal.parser.ParseHelper.PROPERTY_SKILL_LEVEL_UP;
import static com.github.paladijn.d2rsavegameparser.internal.parser.ParseHelper.PROPERTY_UNDEAD_DMG;
...
final class ItemParser {
    ...
    private void parseExtendedPart2(Item.ItemBuilder itemBuilder, ItemScaffolding itemScaffolding, BitReader br) {
        ...
        switch (itemScaffolding.getItemType()) {
            case ARMOR -> parseArmorStats(itemBuilder, br);
            case WEAPON -> parseWeaponStats(itemBuilder, itemScaffolding, br, txtProperties.getWeaponStatsByCode(itemScaffolding.getCode()));
            case MISC -> parseMiscStats(itemBuilder, br, txtProperties.getMiscItemsByCode(itemScaffolding.getCode()));
        }
    ...
    private void parseWeaponStats(Item.ItemBuilder itemBuilder, ItemScaffolding itemScaffolding, BitReader br, WeaponStats weaponStats) {
        ...
        if ("club".equals(itemScaffolding.getType())
            ...
            final ItemStatCost isc = txtProperties.getItemStatCostsByID(PROPERTY_UNDEAD_DMG);
    ...
    private List<ItemProperty> readProperties(BitReader br, int qflag) {
        ...
        if (rootProp == PROPERTY_END) {
            log.debug("skipping properties due to value 511");
        }
        while (rootProp != PROPERTY_END) {
            ...
            if (rootProp == PROPERTY_PHYS_MAX_DMG
                    || rootProp == PROPERTY_FIRE_MIN_DMG
                    || rootProp == PROPERTY_LIGHT_MIN_DMG
                    || rootProp == PROPERTY_MAGIC_MIN_DMG) {
                properties.add(parseItemProperty(br, rootProp + 1, qflag));
            } else if (rootProp == PROPERTY_COLD_MIN_DMG || rootProp == PROPERTY_POISON_MIN_DMG) { // these also include the duration value
                properties.add(parseItemProperty(br, rootProp + 1, qflag));
    ...
    private ItemProperty parseItemProperty(BitReader br, int rootProp, int qflag) {
        ...
        if (rootProp == PROPERTY_SKILL_GET_HIT
                || rootProp == PROPERTY_SKILL_DEATH
                || rootProp == PROPERTY_SKILL_LEVEL_UP
                || rootProp == PROPERTY_SKILL_ATTACK
                || rootProp == PROPERTY_SKILL_HIT
                || rootProp == PROPERTY_SKILL_KILL) {
        ...
        } else if (rootProp == PROPERTY_CHARGED_SKILL) {
    ...
    private ItemScaffolding getBasicItemStats(String code, ItemType itemType, ArmorStats armorStats, WeaponStats weaponStats, final MiscStats miscStats,
        ...
        return switch (itemType) {
            case ARMOR -> new ItemScaffolding(code, armorStats.getName(), armorStats.getType(), armorStats.getType2(), armorStats.getReqStr(),
                    armorStats.getReqDex(), armorStats.getReqLvl(), isPersonalized, isRuneword, isSocketed, isEthereal, itemType);
            case WEAPON -> new ItemScaffolding(code, weaponStats.getName(), weaponStats.getType(), weaponStats.getType2(), weaponStats.getReqStr(),
                    weaponStats.getReqDex(), weaponStats.getReqLvl(), isPersonalized, isRuneword, isSocketed, isEthereal, itemType);
            case MISC -> new ItemScaffolding(code, miscStats.getName(), miscStats.getType(), miscStats.getType2(), miscStats.getReqStr(),
                    miscStats.getReqStr(), miscStats.getReqLvl(), isPersonalized, isRuneword, isSocketed, isEthereal, itemType);
        };
    }
    ...
    private void parseRare(Item.ItemBuilder itemBuilder, ItemScaffolding itemScaffolding, BitReader br) {
        ...
        final String name1 = txtProperties.getRarePrefixById(rareId1 - 156);
